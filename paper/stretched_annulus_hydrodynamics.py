...
import gyropoly.stretched_annulus as domain
from gyropoly.decorators import cached, profile


g_file_prefix = 'stretched_annulus_hydrodynamics'
...
def combined_projection(geometry, m, Lmax, Nmax, alpha, sigma):
    ...
    if geometry.degree == 0:
        ...
        side_shifts = [3,2,1,0]  # total size 4*(Lmax-2) = 4*Lmax-8, top+side = 2*Nmax+2*Lmax-5 ok
...
def build_matrices_galerkin(geometry, m, Lmax, Nmax, Ekman, alpha):
    dL, dN = 2, 2+(2-int(geometry.root_h))*geometry.degree
