...
class OrthogonalPolynomialSystem():
    dtype = 'float64'
    internal = 'float128'
    ...
    @property
    def params(self):
        raise NotImplemented('Derived class has no params implementation')

    @property
    def num_factors(self):
        return len(self.params)

    def factor(self, z, index):
        if index < 0 or index >= self.num_factors:
            raise ValueError('Invalid index')
        return self.factors(z)[index]

    def factors(self, z):
        raise NotImplemented('Derived class has no factors implementation')

    def weight(self, dtype=dtype, internal=internal):
        raise NotImplemented('Derived class has no weight implementation')

    def mass(self, dtype=dtype, internal=internal):
        raise NotImplemented('Derived class has no mass implementation')

    def recurrence(self, n, dtype=dtype, internal=internal):
        raise NotImplemented('Derived class has no recurrence implementation')

    def quadrature(self, n, dtype=dtype, internal=internal):
        raise NotImplemented('Derived class has no quadrature implementation')

    def polynomials(self, n, z, dtype=dtype, internal=internal):
        raise NotImplemented('Derived class has no polynomials implementation')

    def has_even_parity(self):
        return False


class Jacobi(OrthogonalPolynomialSystem):
    dtype = OrthogonalPolynomialSystem.dtype
    internal = OrthogonalPolynomialSystem.internal
    ...
    def weight(self, z):
        return (1-z)**self.a * (1+z)**self.b

    def mass(self, dtype=dtype, internal=internal):
        return jacobi.mass(self.a, self.b).astype(dtype)

    def recurrence(self, n, dtype=dtype, internal=internal):
        return jacobi.operator('Z', dtype=internal)(n, self.a, self.b).astype(dtype)

    def quadrature(self, n, dtype=dtype, internal=internal):
        return jacobi.quadrature(n, self.a, self.b, dtype=dtype, internal=internal)

    def polynomials(self, n, z, dtype=dtype, internal=internal):
        return jacobi.polynomials(n, self.a, self.b, z, dtype=dtype, internal=internal)

    def has_even_parity(self):
        return self.a == self.b    


class AugmentedSystem(OrthogonalPolynomialSystem):
    dtype = OrthogonalPolynomialSystem.dtype
    internal = OrthogonalPolynomialSystem.internal

    def __init__(self, base, rho, c):
        super().__init__()
        self.base, self.rho, self.c = base, rho, c
        self.config = _make_rho_config(rho)

    @property
    def params(self):
        return self.base.params + (self.c,)

    @property
    def num_factors(self):
        return 1+self.base.num_factors

    def factors(self, z):
        return self.base.factors(z) + (self.config['rho'](z),)

    def weight(self, z):
        return self.base.weight(z) * self.config['rho'](z)**self.c

    def mass(self, dtype=dtype, internal=internal, **quadrature_kwargs):
        return mass(self.base, self.rho, self.c, dtype=dtype, internal=internal, **quadrature_kwargs)

    def recurrence(self, n, dtype=dtype, internal=internal, **quadrature_kwargs):
        return recurrence(n, self.base, self.rho, self.c, dtype=dtype, internal=internal, **quadrature_kwargs)

    def quadrature(self, n, dtype=dtype, internal=internal, **quadrature_kwargs):
        return quadrature(n, self.base, self.rho, self.c, dtype=dtype, internal=internal, **quadrature_kwargs)

    def polynomials(self, n, z, dtype=dtype, internal=internal, **quadrature_kwargs):
        return polynomials(n, self.base, self.rho, self.c, z, dtype=dtype, internal=internal, **quadrature_kwargs)

    def has_even_parity(self):
        if self.base.has_even_parity():
            if self.config['is_polynomial'] and self.config['degree'] % 2 == 0:
                if np.all(np.array(rho[1::2]) == 0):
                    return True
        return False 


def build_system(a, b, rhoc):
    system = Jacobi(a, b)
    for rho, c in rhoc:
        system = AugmentedSystem(system, rho, c)
    return system
...
def mass(base_system, rho, c, dtype='float64', internal='float128', **quadrature_kwargs):
    """
    ...
    Parameters
    ----------
    base_system : OrthogonalPolynomialSystem
        Base OP system to augment with additional weight factor rho**c
    ...
    c : float
        rho power in augmented weight function
    ...
    def fun(nquad):
        z, w = base_system.quadrature(nquad, dtype=internal)
        return np.sum(w*rho_fun(z)**c), None

    mu, _ = tools.quadrature_iteration(fun, nquad, max_iters, label='Mass', **quadrature_kwargs)
    return mu.astype(dtype)


def stieltjes(n, base_system, rho, c, return_mass=False, dtype='float64', internal='float128', **quadrature_kwargs):
    """
    ...
    base_quadrature = lambda nquad: base_system.quadrature(nquad, dtype=internal)
...
def modified_chebyshev(n, base_system, rho, c, return_mass=False, dtype='float64', internal='float128', **quadrature_kwargs):
    """
    Compute the three-term recurrence coefficients for the weight function
        w(z) = base_system.weight(z) * rho(z)**c
    ...
    base_operator = lambda n: base_system.recurrence(n, dtype=internal)
    base_quadrature = lambda nquad: base_system.quadrature(nquad, dtype=internal)
    base_polynomials = lambda npoly, z: base_system.polynomials(npoly, z, dtype=internal)
    augmented_weight = lambda z: rho_fun(z)**c
...
def recurrence(n, base_system, rho, c, return_mass=False, dtype='float64', internal='float128', algorithm='stieltjes', **quadrature_kwargs):
    """
    Compute the three-term recurrence coefficients for the weight function
        w(z) = base_system.weight(z) * rho(z)**c
    on the interval (-1,1)
    ...
    """
    if c == 0:
        Z = base_system.recurrence(n, dtype=dtype)
        return (Z, base_system.mass(dtype=dtype)) if return_mass else Z
    ...
    return fun(n, base_system, rho, c, return_mass=return_mass, dtype=dtype, internal=internal, **quadrature_kwargs)


def polynomials(n, base_system, rho, c, z, init=None, dtype='float64', internal='float128', **recurrence_kwargs):
    """
    ...
    """
    if c == 0:
        return base_system.polynomials(n, z, dtype=dtype)

    Z, mass = recurrence(n, base_system, rho, c, return_mass=True, dtype=internal, **recurrence_kwargs)
    return tools.polynomials(Z, mass, z, init=init, dtype=dtype, internal=internal)


def quadrature(n, base_system, rho, c, dtype='float64', internal='float128', **recurrence_kwargs):
    """
    ...
    """
    if c == 0:
        return base_system.quadrature(n, dtype=dtype)

    # Compute the Jacobi operator.  eigs requires double precision inputs
    Z, mass = recurrence(n, base_system, rho, c, dtype='float64', internal=internal, return_mass=True, **recurrence_kwargs)
    return tools.quadrature(Z, mass, dtype=dtype)


def embedding_operator(n, base_system, rho, c, dtype='float64', internal='float128', **recurrence_kwargs):
    """
    ...
    system = AugmentedSystem(base_system, rho, c)
    ...
    cosystem = AugmentedSystem(base_system, rho, c+dc)

    z, w = cosystem.quadrature(n, dtype=internal, **recurrence_kwargs)
...
def embedding_operator_adjoint(n, base_system, rho, c, dtype='float64', internal='float128', **recurrence_kwargs):
    """
