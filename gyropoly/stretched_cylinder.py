...
def tangential_stress(geometry, m, Lmax, Nmax, alpha, direction, dtype='float64', internal='float128', recurrence_kwargs=None):
    """
...
def s_dot(geometry, m, Lmax, Nmax, alpha, sigma=0, exact=True, dtype='float64', internal='float128', recurrence_kwargs=None):
    ...
    radius = geometry.radius
    ones = np.ones(Lmax)
    make_op = lambda sop, dsigma: (radius/2 * _make_operator(geometry, 0, ones, sop, m, Lmax, Nmax, alpha, sigma=sigma+dsigma, Lpad=Lpad, Npad=Npad)).astype(dtype)
...
def s_vector(geometry, m, Lmax, Nmax, alpha, sigma=0, exact=True, dtype='float64', internal='float128', recurrence_kwargs=None):
    ...
    radius = geometry.radius
    ones = np.ones(Lmax)
    make_op = lambda sop: (radius/2 * _make_operator(geometry, 0, ones, sop, m, Lmax, Nmax, alpha, sigma=sigma, Lpad=Lpad, Npad=Npad)).astype(dtype)
